...
#pass the post idea as a url parameter
@app.route('/<int:id>/edit/', methods=('GET', 'POST'))
def edit(id):
    #get the post from the database with a select query from the post with that id
    post = get_post(id)
    ...
    if request.method == 'POST':
        ...
        if not title:
         ...
         #if POST then process the form dat, Get the data and validate it. Update the POST and redirect to homepage
   
    #if GET then display page
    return render_template('edit.html', post=post)

#create a route to delete a post
#delete page will only be proccesed with a POST method
